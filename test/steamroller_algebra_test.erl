...
list_concat_test_() ->
  ...
  Expect2 =
    ...
    >>,
  Result2 = steamroller_algebra:format_tokens(Tokens, 45),
  [
    ?_assertEqual(Expect0, Result0),
...
bool_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"foo(X) -> true = bar(X) =:= (X > 0 andalso X < 10).">>),
...
negative_float_test_() ->
  {ok, Tokens0} = steamroller_ast:tokens(<<"foo() -> -2.0.">>),
  Expect0 = <<"foo() -> -2.0.\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens0, 100),
  {ok, Tokens1} = steamroller_ast:tokens(<<"foo() -> 1 * -2.0.">>),
  Expect1 = <<"foo() -> 1 * -2.0.\n">>,
  Result1 = steamroller_algebra:format_tokens(Tokens1, 100),
  {ok, Tokens2} = steamroller_ast:tokens(<<"foo() -> 1 - 2.0.">>),
  Expect2 = <<"foo() -> 1 - 2.0.\n">>,
  Result2 = steamroller_algebra:format_tokens(Tokens2, 100),
  {ok, Tokens3} = steamroller_ast:tokens(<<"foo(X) -> X - 2.0.">>),
  Expect3 = <<"foo(X) -> X - 2.0.\n">>,
  Result3 = steamroller_algebra:format_tokens(Tokens3, 100),
  {ok, Tokens4} = steamroller_ast:tokens(<<"foo() -> bar() - 2.0.">>),
  Expect4 = <<"foo() -> bar() - 2.0.\n">>,
...
unicode_test_() ->
  {ok, Tokens0} =
    steamroller_ast:tokens(<<"foo() -> 'こんにちは' = bar(\"こんにちは\"), ok."/utf8>>),
  Expect0 = <<"foo() ->\n  'こんにちは' = bar(\"こんにちは\"),\n  ok.\n"/utf8>>,
  Result0 = steamroller_algebra:format_tokens(Tokens0, 100),
...
function_long_multiclause_test_() ->
  ...
  Expect2 =
    <<
      "my_function(atom_1, X) ->\n  {atom_1, X};\n\nmy_function(\n  atom_2,\n  {something, X, other_thing}\n) ->\n  {\n    something,\n    X,\n    yet_another_thing\n  };\n\nmy_function(atom_2, X) -> X.\n"
    >>,
  Result2 = steamroller_algebra:format_tokens(Tokens, 30),
  Expect3 =
    <<
...
function_multiclause_break_test_() ->
  {ok, Tokens0} =
    steamroller_ast:tokens(
      <<
        "foo({ok, X}) -> bar(X), baz();\nfoo({error, a}) -> something_long;\nfoo({error, b}) -> something_else."
      >>
    ),
  Expect0 =
    <<
      "foo({ok, X}) ->\n  bar(X),\n  baz();\n\nfoo({error, a}) ->\n  something_long;\n\nfoo({error, b}) ->\n  something_else.\n"
    >>,
  Result0 = steamroller_algebra:format_tokens(Tokens0, 30),
  {ok, Tokens1} =
    steamroller_ast:tokens(
      <<
        "foo({error, a}) -> something_long;\nfoo({ok, X}) -> bar(X), baz();\nfoo({error, b}) -> something_else."
  ...
  Expect1 =
    <<
      "foo({error, a}) ->\n  something_long;\n\nfoo({ok, X}) ->\n  bar(X),\n  baz();\n\nfoo({error, b}) ->\n  something_else.\n"
    >>,
  Result1 = steamroller_algebra:format_tokens(Tokens1, 30),
  {ok, Tokens2} =
    steamroller_ast:tokens(
      <<
        "foo({error, a}) -> something_long;\nfoo({error, b}) -> something_else;\nfoo({ok, X}) -> bar(X), baz()."
  ...
  Expect2 =
    <<
      "foo({error, a}) ->\n  something_long;\n\nfoo({error, b}) ->\n  something_else;\n\nfoo({ok, X}) ->\n  bar(X),\n  baz().\n"
    >>,
  Result2 = steamroller_algebra:format_tokens(Tokens2, 30),
  [
    ?_assertEqual(Expect0, Result0),
    ?_assertEqual(Expect1, Result1),
    ?_assertEqual(Expect2, Result2)
...
function_multiclause_full_break_test_() ->
  {ok, Tokens0} =
    steamroller_ast:tokens(
      <<
        "foo({ok, X}) -> bar(X), baz();\nfoo({error, a}) -> hello(), something_long;\nfoo({error, b}) -> goodbye(), something_else."
      >>
    ),
  Expect0 =
    <<
...
function_head_pattern_match_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"long_foo({X, _} = Y) -> {long_bar(X), long_baz(Y)}.">>),
  Expect0 = <<"long_foo({X, _} = Y) -> {long_bar(X), long_baz(Y)}.\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens, 100),
  Expect1 = <<"long_foo(\n  {X, _} = Y\n) ->\n  {\n    long_bar(X),\n    long_baz(Y)\n  }.\n">>,
...
long_type_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"-type my_type() :: one | two | three | four | five.">>),
...
spec_comment_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(<<"% My first spec\n\n\n-spec test(some_type()) -> other_type().\n">>),
...
if_attribute_test_() ->
  ...
  {ok, Tokens} =
    steamroller_ast:tokens(<<"-if(something).\n-define(x, 1).\n-else.\n-define(x, 2).\n-endif.">>),
...
case_test_() ->
  ...
  {ok, Tokens1} =
    steamroller_ast:tokens(
      <<
        "foo(X) -> case bar(X) of {error, oops} -> oops; {ok, Y} -> Z = baz(Y), foo(Z); {error, _} -> oh_no end."
      >>
    ),
  Expect1 =
    <<
  ...
  {ok, Tokens2} =
    steamroller_ast:tokens(
      <<
        "foo(X) -> case bar(X) of {error, oops} -> oops; {error, _} -> oh_no; {ok, Y} -> Z = baz(Y), foo(Z) end."
      >>
    ),
  Expect2 =
    <<
...
case_break_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(
      <<
        "foo(X) -> case bar(X) of {ok, Y} -> [bar(Y), baz(Y), bar(X), baz(X)]; {error, oops} -> oops; {error, _} -> oh_no end."
      >>
    ),
  Expect0 =
    <<
      "foo(X) ->\n  case bar(X) of\n    {ok, Y} -> [bar(Y), baz(Y), bar(X), baz(X)];\n    {error, oops} -> oops;\n    {error, _} -> oh_no\n  end.\n"
    >>,
  Result0 = steamroller_algebra:format_tokens(Tokens, 100),
  Expect1 =
    <<
      "foo(X) ->\n  case bar(X) of\n    {ok, Y} ->\n      [bar(Y), baz(Y), bar(X), baz(X)];\n\n    {error, oops} -> oops;\n    {error, _} -> oh_no\n  end.\n"
    >>,
  Result1 = steamroller_algebra:format_tokens(Tokens, 40),
  Expect2 =
    <<
...
matched_case_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(<<"foooooooooo(X) -> Ret = case X of some_value -> great end, Ret.">>),
...
case_fun_var_2_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"foo() -> case X of 1 -> fun X() -> ok, X() end end.">>),
  Expect0 =
    <<
      "foo() ->\n  case X of\n    1 ->\n      fun\n        X() ->\n          ok,\n          X()\n      end\n  end.\n"
    >>,
...
if_test_() ->
  {ok, Tokens0} = steamroller_ast:tokens(<<"foo(A, B) -> if A == B -> great; true -> oh_no end.">>),
  Expect0 = <<"foo(A, B) ->\n  if\n    A == B -> great;\n    true -> oh_no\n  end.\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens0, 100),
  Expect1 = Expect0,
  ...
  Expect2 =
    <<"foo(A, B) ->\n  if\n    A == B ->\n      great;\n\n    true ->\n      oh_no\n  end.\n">>,
  Result2 = steamroller_algebra:format_tokens(Tokens0, 15),
  {ok, Tokens1} =
  ...
  Expect5 =
    <<"foo(A) ->\n  if\n    length(A) > 1 ->\n      great;\n\n    true -> oh_no\n  end.\n">>,
  Result5 = steamroller_algebra:format_tokens(Tokens1, 20),
  [
    ?_assertEqual(Expect0, Result0),
    ?_assertEqual(Expect1, Result1),
    ?_assertEqual(Expect2, Result2),
...
dynamic_function_test_() ->
  ...
  {ok, Tokens1} = steamroller_ast:tokens(<<"foooooooooo(X) -> lists:any(fun module:func/1, X).">>),
...
guarded_case_test_() ->
  ...
  Expect2 =
    ...
    >>,
  Result2 = steamroller_algebra:format_tokens(Tokens, 37),
  [
    ?_assertEqual(Expect0, Result0),
    ?_assertEqual(Expect1, Result1),
    ?_assertEqual(Expect2, Result2)
...
if_guard_sequence_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(<<"foo(X) -> if X =:= test; X =:= not_test -> ok; true -> error end.">>),
  Expect0 =
...
begin_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"-define(macro(Thing),\n begin\n (Thing + 1)\n end).">>),
...
try_catch_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"foo() -> try bar(), baz() catch Y -> {caught, Y} end">>),
  Expect0 = <<"foo() ->\n  try\n    bar(),\n    baz()\n  catch\n    Y -> {caught, Y}\n  end\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens, 100),
  Expect1 = <<"foo() ->\n  try\n    bar(),\n    baz()\n  catch\n    Y -> {caught, Y}\n  end\n">>,
  Result1 = steamroller_algebra:format_tokens(Tokens, 40),
...
macro_case_test_() ->
  ...
  % There should be an indent after the macro.
  Expect2 = <<"foo(X) ->\n  case bar(X) of\n    ?Macro(X)\n    ok\n  end.\n">>,
  Result2 = steamroller_algebra:format_tokens(Tokens, 15),
  [
    ?_assertEqual(Expect0, Result0),
    ?_assertEqual(Expect1, Result1),
    ?_assertEqual(Expect2, Result2)
...
record_definition_fun_test_() ->
  {ok, Tokens} = steamroller_ast:tokens(<<"-record(state, {one :: fun(), two=2 :: integer()}).">>),
...
map_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(<<"foo() -> X = #{test => 123, other => 456}, X#{\"test\" => 789}.">>),
...
function_clause_comment_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(<<"foo(Arg1, Arg1) -> error;\n% Hello World\nfoo(Arg1, Arg2) -> ok.">>),
  Expect0 = <<"foo(Arg1, Arg1) -> error;\n% Hello World\nfoo(Arg1, Arg2) -> ok.\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens, 100),
  Expect1 = <<"foo(Arg1, Arg1) ->\n  error;\n\n% Hello World\nfoo(Arg1, Arg2) ->\n  ok.\n">>,
  Result1 = steamroller_algebra:format_tokens(Tokens, 20),
...
case_comment_sadness_test_() ->
  {ok, Tokens} =
    steamroller_ast:tokens(<<"foo(X) -> case X of hello -> world\n% A very special comment\nend.">>),
  Expect0 =
    <<"foo(X) ->\n  case X of\n    hello -> world\n    % A very special comment\n  end.\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens, 100),
  [?_assertEqual(Expect0, Result0)].
...
case_arg_comment_test_() ->
  {ok, Tokens0} = steamroller_ast:tokens(<<"foo(X) -> case % Comment\n X of hello -> world end.">>),
  Expect0 = <<"foo(X) ->\n  case\n    % Comment\n    X\n  of\n    hello -> world\n  end.\n">>,
  Result0 = steamroller_algebra:format_tokens(Tokens0, 100),
  {ok, Tokens1} =
