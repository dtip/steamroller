...
comment_tokens_test_() ->
    [
     ?_assertEqual([{comment, 0, "% Comment"}], steamroller_ast:tokens(<<"% Comment\n">>)),
     ?_assertEqual([{comment, 0, "% Comment"}], steamroller_ast:tokens(<<"% Comment">>)),
     ?_assertEqual([{comment, 0, "% Comment"},
                    {comment, 1, "%% API"}], steamroller_ast:tokens(<<"% Comment\n%% API\n">>)),
     ?_assertEqual([{comment, 0, "% Comment"},
                    {comment, 1, "%% API"}], steamroller_ast:tokens(<<"% Comment\n%% API">>))
    ].
...
module_attribute_tokens_test_() ->
    [
     ?_assertEqual([{'-',0}, {atom,0,module}, {'(',0}, {atom,0,test}, {')',0}, {dot,0}], steamroller_ast:tokens(<<"-module(test).\n">>)),
     ?_assertEqual([{'-',0}, {atom,0,include_lib}, {'(',0}, {string,0,"eunit/include/eunit.hrl"}, {')',0}, {dot,0}], steamroller_ast:tokens(<<"-include_lib(\"eunit/include/eunit.hrl\").\n">>)),
     ?_assertEqual([{'-',0}, {atom,0,define}, {'(',0}, {var,0,'MACRO'}, {',',0}, {atom,0,some_atom}, {')',0}, {dot,0}], steamroller_ast:tokens(<<"-define(MACRO, some_atom).\n">>)),
     ?_assertEqual([{'-',0}, {atom,0,define}, {'(',0}, {var,0,'MACRO'}, {',',0}, {string,0,"some_string"}, {')',0}, {dot,0}], steamroller_ast:tokens(<<"-define(MACRO, \"some_string\").\n">>))
    ].
    ...
    [
     ?_assertEqual(
        [
         ...
         {'->',0},
         {atom,0,ok},
         {dot,0}], steamroller_ast:tokens(<<"foo(Arg1, Arg1) -> error; foo(Arg1, Arg2) -> ok.">>))
    ].
